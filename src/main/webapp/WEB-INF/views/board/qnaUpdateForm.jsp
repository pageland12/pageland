...
<body onload="togglePassword(document.qna.qsecret.value)">

    <%@ include file="/WEB-INF/views/guest/header.jsp" %>

    <div class="write-container">
        
        <div class="page-title">Q&A 수정</div>
        ...
        <form name="qna" method="post" action="/board/qnaUpdate" enctype="multipart/form-data">
            ...
            <table class="write-table">
                <tr>
                    <th>제목</th>
                    <td>
                        <input type="text" name="qtitle" class="input-text" value="${update.qtitle}">
                    </td>
                </tr>
                <tr>
                    <th>본문</th>
                    <td>
                        <textarea name="qcontent" class="textarea-content" placeholder="내용을 입력해주세요.">${update.qcontent}</textarea>
                    </td>
                </tr>
                <tr>
                    <th>첨부파일</th>
                    <td>
                        ...
                        <c:if test="${not empty update.qfiles}">
                            <div class="current-file-info">기존 파일: ${update.qfiles}</div>
                        </c:if>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <div class="radio-group">
                            <label>
                                <input type="radio" name="qsecret" value="공개글" onchange="togglePassword(this.value)" ${update.qsecret == '공개글' ? 'checked' : ''}> 공개글
                            </label>
                            <label>
                                <input type="radio" name="qsecret" value="비밀글" onchange="togglePassword(this.value)" ${update.qsecret == '비밀글' ? 'checked' : ''}> 비밀글
                            </label>
                        </div>
                    </td>
                </tr>
                <tr id="passwdRow">
                    <th>게시글 비밀번호</th>
                    <td>
                        <input type="password" name="qpasswd" id="qpasswd" class="input-text" placeholder="변경할 비밀번호를 입력하세요 (선택)">
                    </td>
                </tr>
            </table>
