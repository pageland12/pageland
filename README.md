...
| **팀원(Member 2) 최민식** | 회원 관리 / 상품 운영 관리 | • 회원가입, 로그인/로그아웃, 비밀번호 확인 및 회원정보 수정/탈퇴 구현<br>• 관리자 도서 등록/수정/삭제(CRUD) 및 이미지 업로드 관리<br>• 회원 및 상품 운영 관련 Controller, DAO, DTO 설계 및 View 구현 |
| **팀원(Member 3) 허지은** | 상품 전시 / UI / 문서화 | • 도서 목록 페이징(Pagination) 및 카테고리/조건별 필터링 조회 구현<br>• 베스트/추천 도서 등 사용자 중심의 상품 진열(UI/UX) 레이아웃 구성<br>• 프로젝트 기초 도서/이미지 데이터 수집 및 정제<br>• 프로젝트 최종 보고서 및 발표 자료 작성 |
